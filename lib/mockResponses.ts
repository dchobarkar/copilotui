...
/** Build a response with optional subject interpolation */
...
/** Prompt-specific responses for common patterns (checked before category fallback) */
const PROMPT_SPECIFIC: Array<{
  pattern: RegExp;
  responses: string[];
}> = [
  {
...
export function getMockResponse(userPrompt: string): string {
  ...
  for (const { pattern, responses } of PROMPT_SPECIFIC) {
  ...
  // Interpolate subject where applicable (explain category benefits most)
  const interpolated =
    category === "explain" ? withSubject(response, subject) : response;

  // Occasionally echo the prompt subtly for a more "aware" feel (20% chance)
